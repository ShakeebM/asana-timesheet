/**
 * Created by netstager on 03/10/14.
 */

var app=angular.module('myApp',["ngSanitize", "ngCsv"]).filter('millSecondsToTimeString', function() {
...
app.controller('StatiController', function($scope,$http,$filter){

    $scope.filterdhistory_backup=[];
    $scope.filterdhistory=[];
    $scope.userview=0;
    $scope.datechosento='';
    $scope.datechosenfrom='';
    $scope.init= function(){
        if(localStorage.user){
            ...
            $http({method: 'GET',url: 'asana/user/useractions.php?action=getuserdata&uid='+localStorage.user})
    ...
    $scope.getTaskByUser= function (){
        ...
        $http({method: 'GET',url: 'asana/user/useractions.php?action=gettaskdata&userid='+localStorage.user})
            .success(function (data,status){
               // $scope.taskinfo=data;
                var projectlist=[];
    ...
    $scope.findhistoricdata=function(){
        $scope.getTaskByUser();
        var today = new Date();
        var datestarter= today.format("yyyy-m")+'-01';
        var datetoday = today.format("yyyy-m-d");
        ...
        $http({method: 'GET',url: 'asana/user/useractions.php?action=findhistoricdata&userid='+localStorage.user})
            .success(function(data,status){
                $scope.filterdhistory=data;
                $scope.filterdataByproject=data;
                $scope.filterdhistory_backup=data;
                var timetake=0;
                console.log($scope.filterdhistory_backup)
                angular.forEach(data, function(value, key) {
    ...
    //$scope.selectbyTask=function(selectedtask){
    //    var array=[];
    ...
    //    console.log($scope.filterdhistory)
